...
#if 0 // CONFIG_LCD_RGB_ISR_IRAM_SAFE
...
static esp_err_t rgb_panel_remount(esp_lcd_panel_t *panel, size_t offset);
static uint8_t**  rgb_panel_get_fb(esp_lcd_panel_t *panel);
static int * rgb_panel_get_bounce_pos(esp_lcd_panel_t *panel);
...
//static esp_err_t lcd_rgb_panel_create_circular_trans_link(esp_rgb_panel_t *panel);
...
esp_err_t esp_lcd_new_rgb_panel(const esp_lcd_rgb_panel_config_t *rgb_panel_config, esp_lcd_panel_handle_t *ret_panel)
{
...
#if 1 // CONFIG_LCD_RGB_ISR_IRAM_SAFE
            ...
            for (int a=rgb_panel_config->bounce_buffer_size_px; a>0; a--) {
                if ((no_pixels % (a*bpp))==0) {
                    ...
                    ESP_LOGW(TAG, "Adjusted bounce buffer size from %d to %d pixels to fix this.",
                            rgb_panel_config->bounce_buffer_size_px, bounce_bytes/bpp);
    ...
    */
    ...
    memcpy(rgb_panel->data_gpio_nums, rgb_panel_config->data_gpio_nums, rgb_panel->data_width); // SOC_LCD_RGB_DATA_WIDTH);
    ...
    rgb_panel->base.remount = rgb_panel_remount;
    rgb_panel->base.get_fb = rgb_panel_get_fb;
    rgb_panel->base.get_bounce_pos = rgb_panel_get_bounce_pos;
...
static int * rgb_panel_get_bounce_pos(esp_lcd_panel_t *panel) {
    esp_rgb_panel_t *rgb_panel = __containerof(panel, esp_rgb_panel_t, base);
    return &(rgb_panel->bounce_pos_px);    
}
...
static esp_err_t lcd_rgb_panel_configure_gpio(esp_rgb_panel_t *panel, const esp_lcd_rgb_panel_config_t *panel_config)
{
    ...
    if(panel_config->data_width==8) {
        for(size_t i=8;i<16;i++) {
            gpio_hal_iomux_func_sel(GPIO_PIN_MUX_REG[panel_config->data_gpio_nums[i]], PIN_FUNC_GPIO);
            gpio_set_direction(panel_config->data_gpio_nums[i], GPIO_MODE_OUTPUT);
            gpio_set_level(panel_config->data_gpio_nums[i], 0);
        }
    }
...
static esp_err_t lcd_rgb_panel_create_circular_trans_link(esp_rgb_panel_t *panel)
{
    ...
    // len is ignored
    lcd_com_mount_circular_dma_data(panel->dma_nodes, panel->fb, 0);
...
static IRAM_ATTR bool lcd_rgb_panel_fill_bounce_buffer(esp_rgb_panel_t *panel, uint8_t *buffer)
{
    ...
    uint8_t bpp = (panel->data_width / 8);
    if (panel->on_bounce_empty) {
        ...
        memcpy(buffer, &panel->fb[panel->bounce_pos_px*bpp], panel->bounce_buffer_size_bytes);
        ...
        //the data back
        Cache_WriteBack_Addr((uint32_t)&panel->fb[panel->bounce_pos_px*bpp], panel->bounce_buffer_size_bytes);
        ...
        Cache_Invalidate_Addr((uint32_t)&panel->fb[panel->bounce_pos_px*bpp], panel->bounce_buffer_size_bytes);
    }
    panel->bounce_pos_px+=(panel->bounce_buffer_size_bytes/bpp);
    //If the bounce pos is larger than the framebuffer size, wrap around so the next isr starts pre-loading
    //the next frame.
    if (panel->bounce_pos_px >= panel->fb_size/bpp) {
        panel->bounce_pos_px=0;
    }
    if (!panel->on_bounce_empty) {
        //Preload the next bit of buffer into psram.
        Cache_Start_DCache_Preload((uint32_t)&panel->fb[panel->bounce_pos_px*bpp],
